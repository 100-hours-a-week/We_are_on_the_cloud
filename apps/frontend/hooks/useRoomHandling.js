...
export const useRoomHandling = (
    ...
    const MESSAGE_RETRY_DELAY = 1500;
    ...
    const setupRoom = useCallback(async () => {
        ...
        setupPromiseRef.current = (async () => {
            try {
                ...
                // 1. Socket Setup with retry
                let socketSetupAttempts = 0;
                const maxSocketSetupAttempts = 3;

                while (socketSetupAttempts < maxSocketSetupAttempts) {
                    try {
                        socketRef.current = await setupSocket();
                        break;
                    } catch (error) {
                        socketSetupAttempts++;
                        console.log(`Socket setup attempt ${socketSetupAttempts} failed:`, error);
                        if (socketSetupAttempts >= maxSocketSetupAttempts) {
